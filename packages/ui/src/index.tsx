...
export function VerdictCard({claim}:{claim:Claim}){
  ...
  return <article className={`verity-card state-${claim.state.toLowerCase()}`} aria-live="polite">
    <header className="verity-header">
      <small>{claim.fixture_mode?"Disclosed fixture mode":"Live evidence check"}</small>
    ...
    <section aria-labelledby="verity-claim">
      ...
      <p>{claim.speaker_label} · <time>{(claim.start_ms/1000).toFixed(1)}s</time></p>
    </section>
    ...
    {verdict&&<>
      <section className="interpretation" aria-labelledby="verity-interpretation">
        <h2 id="verity-interpretation">Verity’s evidence-grounded interpretation</h2>
...
export function StatusCard({state}:{state:string}){
  return <section className="verity-card" role="status"><small>Verity</small><h1>{labels[state]??state}</h1></section>;
}
