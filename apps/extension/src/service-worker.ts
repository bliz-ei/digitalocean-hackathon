...
chrome.runtime.onMessage.addListener((message, _sender, reply) => {
  if (message.type === "START_FIXTURE") {
    void startFixture().then(() => reply({ ok: true })).catch((error) => reply({ ok: false, error: safeError(error) }));
    return true;
  }
  if (message.type === "START_LIVE") {
    void startLive().then(() => reply({ ok: true })).catch(async (error) => {
      await update({ error: safeError(error), connection: "ERROR" });
...
async function startFixture(): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    socket.addEventListener("open", () => {
      socket.send(JSON.stringify({ type: "start_fixture", schema_version: "2", session_id: session.id, sequence: 1, payload: {} }));
      resolve();
    }, { once: true });
    socket.addEventListener("error", () => reject(new Error("Unable to open fixture stream")), { once: true });
    socket.addEventListener("message", (event) => void consumeFixtureEvent(socket, session.id, event.data));
    socket.addEventListener("close", () => { if (fixtureSocket === socket) fixtureSocket = undefined; });
  });
}

async function consumeFixtureEvent(socket: WebSocket, sessionId: string, raw: string): Promise<void> {
  const event = JSON.parse(raw) as { type: string; payload?: { public_id?: string; state?: ClaimState; code?: string } };
  if (event.type === "error") {
    await update({ connection: "ERROR", error: event.payload?.code ?? "Fixture stream failed" });
    socket.close();
    return;
  }
  if (event.type !== "claim_state" || !event.payload?.state) return;
  const { public_id: publicId, state } = event.payload;
  await update({ mode: "fixture", connection: state });
  if (!publicId || !terminalStates.has(state)) return;
  const claim = await api.getClaim(base, publicId);
  await update({ mode: "fixture", connection: state, claim });
  socket.close();
}
...
async function stop(): Promise<void> {
  fixtureSocket?.close(); fixtureSocket = undefined;
...
async function current(): Promise<OverlayState> { const { overlayState } = await chrome.storage.session.get("overlayState"); return overlayState ?? { mode: "live", connection: "CONNECTING", transcripts: [] }; }
async function update(state: Partial<OverlayState>): Promise<void> { const next = { ...await current(), ...state }; await chrome.storage.session.set({ overlayState: next }); await broadcast({ type: "OVERLAY_STATE", state: next }); }
async function broadcast(message: unknown): Promise<void> { for (const tab of await chrome.tabs.query({ url: "https://www.youtube.com/*" })) if (tab.id) await chrome.tabs.sendMessage(tab.id, message).catch(() => undefined); }
function validProvider(value: unknown): value is ProviderConfig { if (!value || typeof value !== "object") return false; const item = value as Record<string, unknown>; if (typeof item.baseUrl !== "string" || typeof item.apiKey !== "string" || typeof item.model !== "string") return false; try { return ["https://api.openai.com", "https://inference.do-ai.run"].includes(new URL(item.baseUrl).origin); } catch { return false; } }
