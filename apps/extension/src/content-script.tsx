...
function Overlay({state}:{state?:OverlayState}){
  ...
  if(state.claim?.verdict)return <VerdictCard claim={state.claim}/>;
...
function mount(state?:OverlayState){
  ...
  if(!target){
    ...
    style.textContent=":host{all:initial}.shell{position:fixed;z-index:2147483647;right:20px;top:76px;width:min(360px,calc(100vw - 40px));box-sizing:border-box;padding:16px;border:1px solid #bed1c4;border-radius:14px;background:#fff;color:#17211b;font:14px/1.45 system-ui;box-shadow:0 8px 30px #0003}.shell h2{margin:.35rem 0}.shell ol{padding-left:1.25rem;max-height:240px;overflow:auto}.shell li{margin:.55rem 0}.shell blockquote{margin:.7rem 0;padding:.7rem;border-left:3px solid #145c3d;background:#f2f7f2}.shell [role=alert]{color:#8d2020}";
