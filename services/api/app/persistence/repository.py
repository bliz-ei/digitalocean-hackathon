...
class MemoryRepository(Repository):
    def __init__(self): self.sessions={}; self.claims={}
...
class SQLiteRepository(Repository):
    def __init__(self,path:Path):
        self.db=sqlite3.connect(path,check_same_thread=False); self.db.execute("create table if not exists sessions(id text primary key,idempotency_key text unique)"); self.db.execute("create table if not exists claims(public_id text primary key, body text not null)")
